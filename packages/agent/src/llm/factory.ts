...
export type FactoryOptions = {
  ...
  maxRetries?: number;
  roleId?: string;
  /** 仅 provider=openai 生效：true => Responses API；false => Chat Completions；undefined => 取 env */
...
export async function streamLLM(
  ...
  if (!client.stream) {
    const env = getEnv();
    const provider = opts?.provider ?? env.AI_PROVIDER;
    throw new LLMConfigurationError(`provider=${provider} 未实现流式输出`);
  }
...
export async function runPromptWithTemplate({
  ...
  const finalSystem = systemPrompt ?? '保持清晰、简洁的回答。';
